import { unit, ease } from './choreography.js';
const ink = (t) => t.color || (t.flourish === 'crimson' ? '#873a30' : t.flourish === 'phantom' ? '#3c5558' : '#303d38');
...
function ribbon(ctx, x, y, r, rotation, progress, thickness, color) {
    ctx.save();
    ctx.translate(x, y);
    ctx.rotate(rotation);
    ctx.scale(1, .64);
    ...
    ctx.fillStyle = color;
    ctx.fill();
    ctx.strokeStyle = 'rgba(250,248,233,.72)';
    ctx.lineWidth = 1.3;
    ctx.beginPath();
...
function wavefront(ctx, r, width, color, rotation = 0) {
    ctx.save();
    ctx.rotate(rotation);
    ctx.fillStyle = color;
    ctx.beginPath();
    ...
    ctx.bezierCurveTo(r * .75 - width, r * .5, r * .75 - width, -r * .5, -r * .3, -r);
    ctx.fill();
    ctx.strokeStyle = 'rgba(250,248,233,.8)';
    ctx.lineWidth = 1;
    ctx.beginPath();
    ctx.moveTo(-r * .3, -r);
    ctx.bezierCurveTo(r * .75, -r * .5, r * .75, r * .5, -r * .3, r);
    ctx.stroke();
    ctx.restore();
}
export function drawPreparation(ctx, t, x, dir, local, ranged) {
    if (local < 140 || local > 950)
        return;
    const phase = unit((local - 140) / 610), fade = Math.sin(phase * Math.PI);
    ctx.save();
    ctx.globalAlpha = fade * (t.weight >= 1.5 ? .55 : .22);
    ctx.strokeStyle = ink(t);
    ctx.lineWidth = 1;
    for (let i = 0; i < 7; i++) {
    ...
    if (t.weight >= 1.5) {
        ctx.beginPath();
        ctx.ellipse(x, 327, 40 + phase * 30, 5 + phase * 3, 0, 0, Math.PI * 2);
        ctx.stroke();
        if (ranged)
            ribbon(ctx, x + dir * 12, 244, 30 + phase * 25, -phase * 2, phase, 4, ink(t));
    }
    ctx.restore();
}
export function drawTechnique(ctx, t, weapon, origin, target, dir, age, force = 0) {
    if (age < 80 || age > 830)
        return;
    ...
    if (weapon === 'needles') {
        ...
        ctx.save();
        ctx.translate(x, target[1]);
        ctx.scale(dir, 1);
        for (let i = 0; i < 3; i++) {
            ctx.translate(-12, 0);
            wavefront(ctx, 28 + i * 9, 3 * weight * p, color);
        }
        ctx.restore();
    }
    else {
        ...
        ctx.save();
        ctx.translate(x, origin[1] + (target[1] - origin[1]) * p);
        ctx.scale(dir, 1);
        ...
        if (point) {
            ...
            for (let i = 0; i < count; i++) {
                ...
                ctx.fillStyle = color;
                ctx.beginPath();
                ctx.moveTo(52 * p, offset);
                ctx.quadraticCurveTo(-reach * .35, offset - 5 * weight, -reach, offset - 2);
                ctx.quadraticCurveTo(-reach * .2, offset + 9 * weight, 52 * p, offset);
                ctx.fill();
        ...
        if (t.trail === 'vortex' || t.pattern === 'spiral') {
            for (let i = 0; i < 3; i++)
                ribbon(ctx, -10, 0, 30 + i * 20, age / 180 + i * 2, p, 3, color);
        }
        ctx.restore();
    }
    if (t.flourish || t.trail === 'dust' || t.weight >= 1.5) {
        ...
        for (let i = 0; i < 14; i++) {
            ...
            ctx.fillStyle = color;
            ctx.beginPath();
            ctx.ellipse(x, y, t.flourish === 'plum' ? 4 : 2, 1.3, a + p, 0, Math.PI * 2);
            ctx.fill();
        }
    }
    ctx.restore();
}
export function drawImpact(ctx, x, y, dir, age, impact, seed) {
    if (!impact.hit || age < 0 || age > 650)
        return;
    ...
    for (let i = 0; i < count; i++) {
        ...
        ctx.fillStyle = color;
        ctx.beginPath();
        ctx.ellipse(px, py, 1.4 + (i % 3) * 1.2, 1 + i % 2, a, 0, Math.PI * 2);
        ctx.fill();
        if (i % 3 === 0)
            stroke(ctx, [px * .5, py * .5], [px, py], color, 1.2);
    }
    if (impact.heavy) {
        ctx.save();
        ctx.scale(dir, 1);
        ...
        ctx.restore();
        ctx.strokeStyle = 'rgba(56,63,51,.4)';
        ctx.lineWidth = 1.5;
        ctx.beginPath();
        ctx.ellipse(0, 324 - y, 25 + burst * 95, 4 + burst * 10, 0, 0, Math.PI * 2);
        ctx.stroke();
        for (let i = 0; i < 6; i++)
            stroke(ctx, [(i - 3) * 12, 324 - y], [(i - 3) * (15 + burst * 13), 324 - y - burst * (6 + i % 3 * 7)], color, 1);
    }
    ctx.restore();
}
...
export function drawSampleAccent(ctx, kind, tip, target, dir, age, impact) {
    if (age < -150 || age > 460)
        return;
    ...
    ctx.beginPath();
    ctx.moveTo(16 * charge, 0);
    ctx.bezierCurveTo(-length * .25, -13 * power, -length * .65, -8 * power, -length, 1);
    ctx.bezierCurveTo(-length * .55, 11 * power, -length * .2, 7 * power, 16 * charge, 0);
    ctx.fill();
    ctx.shadowColor = snow ? '#b8edff' : '#e4eadc';
    ctx.shadowBlur = snow ? 10 : 3;
    ...
    if (kind !== 'sword') {
        for (let i = 0; i < 3; i++) {
            ...
            ctx.beginPath();
            ctx.strokeStyle = '#70867d';
            ctx.lineWidth = 2 - i * .4;
            ctx.ellipse(-i * 14, 0, 8 + i * 4, (18 + i * 9) * charge, 0, -1.15, 1.15);
            ctx.stroke();
        }
    }
    if (snow) {
        for (let i = 0; i < 24; i++) {
            ...
            ctx.strokeStyle = i % 3 ? '#eefbff' : '#70b7d0';
            ctx.fillStyle = '#eefbff';
            ctx.lineWidth = 1;
            if (i % 3 === 0) {
                ctx.save();
                ctx.translate(x, y);
                ctx.rotate(i + age / 650);
    ...
    ctx.scale(dir, 1);
    ctx.globalAlpha = hitFade * .65;
    ctx.strokeStyle = snow ? '#98d9ec' : '#72877d';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.ellipse(5 + burst * 16, 0, 8 + burst * 18, (16 + burst * 35) * power, 0, -1.25, 1.25);
    ctx.stroke();
