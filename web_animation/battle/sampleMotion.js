...
export function sampleMove(event) {
    if (event?.martialArtId === 'sword_falling_plum' && event.move === '寒枝点雪')
        return 'sword';
    if (event?.martialArtId === 'staff_yuejiaqiang' && event.move === '绝招·回马枪')
        return 'spear';
    if (event?.martialArtId === 'leg_shadow_whirl' && event.move === '穿云踢')
        return 'kick';
}
// Clamp unreachable targets, not bone lengths. The pole chooses the elbow/knee side.
export function twoBone(root, target, length, pole) {
...
export function walkContacts(progress, distance) {
    ...
    for (let i = 0; i <= index; i++) {
        ...
        if (i < index)
            ...
            feet[leg] = [mix(feet[leg][0], end, f), -Math.sin(Math.PI * f) * 48];
    }
    return { root: (index + f) * distance / count, feet };
}
export function sampleMotion(kind, time, travel, missed = false) {
    ...
    let feet = run.feet.map(p => [p[0] - x, p[1]]);
    ...
    let front = [61 + Math.sin(cadence) * 18 * running, -116 - Math.cos(cadence) * 18 * running];
    let rear = [-47 - Math.sin(cadence) * 12 * running, -116 + Math.cos(cadence) * 20 * running];
    let angle = -.45;
    ...
    if (kind === 'sword') {
        ...
        if (t >= runEnd) {
            ...
            const kneeUp = [29, -40], extended = [79, -35];
    ...
    if (t >= 1240) {
        ...
        x = travel * (1 - ease(flight));
        lift = -52 * Math.sin(Math.PI * flight);
        ...
        front = point(front, [54, -144], entry);
        rear = point(rear, [-63, -122], entry);
        angle = mix(angle, kind === 'kick' ? 0 : -.55, entry);
        // Recover below the shoulder, not through the arm's IK anchor.
        if (kind === 'sword')
            front[1] += 30 * Math.sin(Math.PI * entry);
    }
    // The hip moves first; shoulder rotation and hand extension are delayed above.
    const neck = [hip[0] + tilt, hip[1] - Math.sqrt(52 * 52 - tilt * tilt)];
    ...
    if (kind === 'spear') {
        ...
        const center = [neck[0] + mix(mix(-6, 48, thrust), 8, returnGrip), neck[1] + mix(mix(44, 24, turn), 28, returnGrip)];
    ...
    const pose = { points: [[neck[0], neck[1] - 21], neck, hip, backElbow, backHand, elbow, hand, backKnee, backFoot, knee, foot], sword: angle, lean: 0, shoulders };
