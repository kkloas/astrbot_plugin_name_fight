export function drawWeapon(ctx, weapon, ink, time) {
    ...
    if (weapon === 'whip') {
        line(-15, 0, 10, 0, 5, '#514c46');
        ctx.strokeStyle = '#b1c0b7'; ctx.lineWidth = 3;
        ctx.beginPath(); ctx.moveTo(8, 0);
        for (let i = 1; i <= 30; i++) {
            const x = 8 + i * 5.7;
            ctx.lineTo(x, Math.sin(i * .27 - time / 100) * Math.sin(i / 30 * Math.PI) * 19);
        }
        ctx.stroke();
        line(174, 0, 184, 0, 1.4, '#727f78');
    } else if (weapon === 'tokens') {
        ctx.fillStyle = '#8b9395'; ctx.strokeStyle = '#30383b'; ctx.lineWidth = 1.5;
        ctx.beginPath(); ctx.moveTo(-10, -5); ctx.lineTo(32, -8); ctx.lineTo(43, 0);
        ctx.lineTo(32, 8); ctx.lineTo(-10, 5); ctx.closePath(); ctx.fill(); ctx.stroke();
        line(2, 0, 30, 0, 2, '#bd542f');
    } else if (weapon === 'heavy_sword') {
