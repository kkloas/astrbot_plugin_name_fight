...
import { themeColors } from './moveProfiles.js';
...
export function drawMoveTheme(ctx, p, tip, target, dir, age, impact) {
    if (age < -300 || age > 570)
        return;
    ...
    ctx.fillStyle = color;
    ctx.lineCap = 'round';
    const curve = (x, y, w, h, width) => {
        ctx.lineWidth = width;
        ctx.beginPath();
        ctx.moveTo(x - dir * w, y + h);
        ctx.quadraticCurveTo(x + dir * w * .55, y - h, x + dir * w, y);
        ctx.stroke();
    };
    ...
    if (p.theme === 'petal' || p.theme === 'leaf' || p.theme === 'snow') {
        for (let i = 0; i < particleCount; i++) {
            ...
            if (p.theme === 'snow') {
                ...
                ctx.quadraticCurveTo(1, p.theme === 'leaf' ? 2 : 6, -5, 0);
                ctx.fill();
            }
            ctx.restore();
        }
    ...
    if (p.theme === 'cloud' || p.theme === 'mist' || p.theme === 'wind') {
        for (let i = 0; i < 6; i++) {
            ...
            ctx.globalAlpha = fade * (.16 - i * .018);
            curve(x, y, 48 + life * 32, 13 + i * 3, 8 - i);
            ctx.globalAlpha = fade * .32;
            curve(x, y, 48 + life * 32, 13 + i * 3, 1);
        }
    }
    if (p.theme === 'jade' || p.style === 'coil') {
        ...
        for (let i = 0; i < 4; i++) {
            ...
            ctx.lineWidth = 2;
            ctx.globalAlpha = fade * (.6 - i * .1);
            ctx.beginPath();
            ctx.ellipse(x, tip[1], radius * .6, radius, dir * life, life * 5 + i, life * 5 + i + Math.PI * 1.3);
            ctx.stroke();
        }
    }
    if (p.theme === 'ink' || p.theme === 'vermilion' || p.style === 'hook') {
        ...
        for (let i = 0; i < 12; i++) {
            ctx.beginPath();
            ctx.ellipse(tip[0] + dir * (noise(i + 9) - .7) * 95, tip[1] + (noise(i + 38) - .5) * 60, 1 + noise(i) * 3, 1 + noise(i + 4) * 2, i, 0, Math.PI * 2);
            ctx.fill();
        }
    }
    if (p.theme === 'gold' || p.theme === 'silver') {
        ...
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.moveTo(tip[0] - dir * length, tip[1] + 3);
        ctx.lineTo(tip[0] + dir * 22, tip[1]);
        ctx.stroke();
        ctx.strokeStyle = '#f7f8ef';
        ctx.lineWidth = 1.6;
        ctx.stroke();
        ctx.strokeStyle = color;
        for (let i = 0; i < 10; i++) {
            const x = tip[0] - dir * noise(i + 4) * length, y = tip[1] + (noise(i + 33) - .5) * 50;
            curve(x, y, 5 + life * 9, 2, 1);
        }
    }
    if (p.theme === 'sound') {
        ...
        for (let i = 0; i < count; i++) {
            ...
            ctx.globalAlpha = fade * (1 - i / (count + 1)) * .75;
            ctx.lineWidth = i === 0 ? 4 : 1.5;
            ctx.beginPath();
            ctx.moveTo(x - dir * 10, tip[1] - radius);
            ctx.quadraticCurveTo(x + dir * radius * .95, tip[1], x - dir * 10, tip[1] + radius);
            ctx.stroke();
        }
    }
    if ((p.style === 'palm' || p.style === 'shock') && age > -100) {
        ...
        for (let i = 0; i < 3; i++) {
            const radius = (25 + life * 65 + i * 12) * strength;
            ctx.lineWidth = 3 - i * .7;
            ctx.beginPath();
            ctx.ellipse(tip[0] + dir * 20, tip[1], radius * .35, radius, 0, -1.3, 1.3);
            ctx.stroke();
        }
    }
    // Decorative motion can play on misses. A contact burst requires actual damage.
    if (impact.hit && age >= 0 && age < 310) {
        const progress = age / 310;
        ...
        for (let i = 0; i < Math.round(10 + impact.force * 14); i++) {
            ...
            ctx.lineWidth = 1 + noise(i) * 2;
            ctx.beginPath();
            ctx.moveTo(target[0] + Math.cos(angle) * reach * .4, target[1] + Math.sin(angle) * reach * .4);
            ctx.lineTo(target[0] + Math.cos(angle) * reach, target[1] + Math.sin(angle) * reach);
            ctx.stroke();
        }
    }
    ctx.restore();
