...
import { twoBone, sampleMotion } from './sampleMotion.js';
const guard = [0, 0, 0, 78, 0, .4, 2.6, 58, -.55];
const end = [1740, 0, 0, 78, 0, .4, 2.6, 58, -.55];
const scores = {
    '有凤来仪': [
...
export function swordSequenceFor(event) {
    if (event?.martialArtId === 'sword_huashan' || event?.martialArtId === 'sword_falling_plum' && event.move === '回风斩')
        return scores[event.move || ''];
}
export function swordSequence(event, t, travel, missed = false) {
    ...
    for (let i = 1; i < keys.length; i++)
        if (t < keys[i][0]) {
            ...
            k = a.map((v, j) => v + (b[j] - v) * f);
            break;
        }
    const [, progress, lift, height, tilt, frontAngle, rearAngle, extension, angle] = k;
    const hip = [0, -height], neck = [tilt, -height - Math.sqrt(52 * 52 - tilt * tilt)];
    const shoulders = [[neck[0] - 10, neck[1] + 6], [neck[0] + 10, neck[1] + 6]];
    const target = (root, a, r) => [root[0] + Math.cos(a) * r, root[1] + Math.sin(a) * r];
    ...
    const footTarget = (index, groundX, airX, airY) => [
    ...
    const pose = { points: [[neck[0], neck[1] - 21], neck, hip, backElbow, backHand, elbow, hand, backKnee, backFoot, knee, foot], shoulders, sword, lean: 0 };
