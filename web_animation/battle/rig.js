import { twoBone, rigLengths } from './sampleMotion.js';
const direction = (a, b, length) => {
...
export function constrainRig(source) {
    const p = source.points.map(v => [...v]);
    p[1] = direction(p[2], p[1], rigLengths.torso);
    p[0] = direction(p[1], p[0], 21);
    const shoulders = [[p[1][0] - 10, p[1][1] + 6], [p[1][0] + 10, p[1][1] + 6]];
