...
const moves=[...samples,...Object.entries(moveProfiles).flatMap(([id,profiles])=>Object.keys(profiles)
  .filter(move=>!samples.some(m=>m.id===id && m.move===move))
  .map(move=>({id,name:arts[id][0],type:arts[id][1],move})))];
...
function MotionPreview() {
  ...
  const battle=useMemo<ReplayBattle>(()=>{
    ...
    return {attacker:side==='a'?sample:opponent,defender:side==='b'?sample:opponent,winner:null,events:[
      ...
      missed?{type:'dodge',time:1250,action:1,actor:side,target}:
        {type:'damage',time:1250,action:1,actor:side,target,cause:'strike',amount:selection===1?340:140,hpAfter:selection===1?660:860,maxHp:1000,bodyPartKey:'chest'},
  ...
  return <main className="motion-preview">
    ...
    <div className="preview-controls">
      ...
      <label>动画 <select aria-label="动画版本" value={motionMode} onChange={e=>setMotionMode(e.target.value as MotionMode)}><option value="mixed">随机混用</option><option value="legacy">旧版</option><option value="current">新版</option></select></label>
