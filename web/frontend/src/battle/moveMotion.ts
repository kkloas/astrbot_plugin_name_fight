...
import { sampleMotion, sampleMove, twoBone, rigLengths, type RigPose } from './sampleMotion';
import { profileFor, type MoveProfile } from './moveProfiles';
...
const pt=(a:Point,b:Point,t:number):Point=>[a[0]+(b[0]-a[0])*t,a[1]+(b[1]-a[1])*t];
const beat=(front:Point,rear:Point,angle:number,tilt=8):Beat=>({front,rear,angle,tilt});
function arc(a:Point,b:Point,t:number,root:Point):Point {
...
function between(a:Beat,b:Beat,t:number):Beat {
...
function sample(keys:[number,Beat][],t:number) {
  for(let i=1;i<keys.length;i++) if(t<keys[i][0]) return between(keys[i-1][1],keys[i][1],ease((t-keys[i-1][0])/(keys[i][0]-keys[i-1][0])));
...
function score(p:MoveProfile,t:number):Beat {
  ...
  switch(p.style) {
    case 'thrust':prep=beat([-18,37],[-60,12],-.1,-7);contact=thrust;follow=thrust;break;
    case 'cross':prep=v===1?low:overhead;contact=slash;break;
    case 'rise':prep=low;contact=beat([78,28],[-62,10],-.13,20);follow=high;break;
    case 'cleave':prep=overhead;contact=beat([75,13],[-46,-8],.04,23);break;
    case 'sweep':prep=beat([-25,40],[-40,8],-2.85,-10);contact=beat([77,56],[-65,5],.02,19);follow=beat([52,47],[-40,15],.4,7);break;
    case 'spin':prep=back;contact=beat([78,14+v*9],[-67,-5],.12,20);break;
    case 'flurry':prep=beat([16,-44],[-45,15],-1.8,-8);contact=v===2?thrust:slash;follow=high;break;
    case 'leap':prep=beat([28,-40],[-63,12],-1.85,-9);contact=v===1?thrust:slash;follow=high;break;
    case 'draw':prep=beat([-14,51],[-36,50],-2.8,-12);contact=beat([83,10],[-66,-7],-.06,22);follow=slash;break;
    case 'palm':case 'shock':prep=beat([-18,50],[-48,24],0,-7);contact=beat([84,22],[-66,-3],0,24);follow=contact;break;
    case 'kick':prep=guard;contact=beat([47,-28],[-72,-12],0,-20);follow=guard;break;
    case 'hook':prep=beat([-18,40],[-50,20],-2.2,-5);contact=beat([70,10+v*8],[-59,10],-.16,18);follow=beat([40,44],[-45,8],1,8);break;
    case 'coil':prep=beat([12,-36],[-58,18],-1.8,-7);contact=beat([72,28],[-58,8],.25,18);follow=beat([-18,46],[-45,20],-2.6,-6);break;
    case 'fan':prep=beat([48,-30],[-58,-26],-.4,-4);contact=beat([80,18],[-69,17],0,12);follow=guard;break;
    case 'needle':prep=beat([-24,40],[-52,25],0,-9);contact=beat([83,8-v*8],[-60,10],0,15);follow=guard;break;
    case 'rain':prep=beat([48,-43],[-60,-24],0,-6);contact=beat([79,9],[-66,10],0,14);follow=guard;break;
    case 'pluck':prep=beat([10,47],[-35,42],0,-3);contact=beat([43+v*7,40],[-35,42],0,6);follow=prep;break;
    case 'strum':prep=beat([-15,20],[-36,42],0,-8);contact=beat([64,46],[-36,42],0,13);follow=guard;break;
    case 'crescendo':prep=beat([7,-25],[-36,42],0,-10);contact=beat([72,30],[-36,42],0,19);follow=beat([45,52],[-36,42],0,7);break;
  }
  const keys:[number,Beat][]=[[0,guard],[260,guard],[a,prep]];
  if(p.style==='flurry') {
...
export function moveMotion(event:BattleEvent,t:number,travel:number,weapon:Weapon,missed=false) {
  ...
  if(named) return sampleMotion(named,t,travel,missed);
  ...
  const hip:Point=[original.points[2][0],ranged?-75:original.points[2][1]];
  const sweep=event.weaponType==='leg' && profile.style==='sweep'?ease((t-760)/150)*(1-ease((t-1050)/150)):0;
  hip[1]+=24*sweep;
  const rotation=b.tilt*weight;
  const neck:Point=[hip[0]+rotation,hip[1]-Math.sqrt(52*52-rotation*rotation)];
  const shoulders:[Point,Point]=[[neck[0]-10,neck[1]+6],[neck[0]+10,neck[1]+6]];
  let front:Point=[neck[0]+b.front[0],neck[1]+b.front[1]];
  let rear:Point=[neck[0]+b.rear[0],neck[1]+b.rear[1]];
  let angle=b.angle;
  let feet:[Point,Point]=[[...original.points[8]],[...original.points[10]]];
  if(ranged) {base.x=0;base.lift=0;feet=[[-41,0],[41,0]];}
  if(profile.style==='draw' && t<1240) {
    base.x=Math.max(0,travel-28)*ease((t-760)/190);
    if(t<760) {base.lift=0;feet=[[-41,0],[41,0]];}
    else {base.lift=-24*Math.sin(Math.PI*unit((t-760)/190));const fold=ease((t-760)/90);feet=[pt([-41,0],[-68,-10],fold),pt([41,0],[47,-7],fold)];}
    const landing=ease((t-950)/120);
    feet=feet.map((f,i)=>pt(f,[i===0?-47:42,0],landing)) as [Point,Point];
  }
  ...
  front=arc(original.points[6],front,weight,shoulders[1]);rear=arc(original.points[4],rear,weight,shoulders[0]);
  if(weapon==='spear') {
    ...
    const center:Point=[neck[0]+12+(40*push-12)*thrust,neck[1]+36-20*push-62*overhead];
    ...
    for(let i=0;i<8;i++) for(const sign of [-1,1]) {
      ...
      if(distance>77) {center[0]-=dx*(1-77/distance);center[1]-=dy*(1-77/distance);}
    }
  ...
  const bladeLength=({sword:99,blade:107,katana:112,spear:140,brush:45} as Partial<Record<Weapon,number>>)[weapon];
  if(bladeLength && weapon!=='spear' && angle>0 && angle<Math.PI/2) angle=Math.min(angle,Math.asin(unit((-hand[1]-5)/bladeLength)));
  const pose:RigPose={points:[[neck[0],neck[1]-21],neck,hip,backElbow,backHand,elbow,hand,backKnee,backFoot,knee,foot],shoulders,sword:angle,lean:0};
  return {...base,pose};
}
