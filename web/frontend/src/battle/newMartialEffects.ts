...
export type NewEffectFrame={hand:Point;tip:Point;root:Point};
type EffectScore={kind:NewEffectKind;power:number;strokes:[number,number][];signature?:boolean};
...
function featherParticle(ctx:CanvasRenderingContext2D,x:number,y:number,angle:number,size:number,accent:string) {
  ctx.save();ctx.translate(x,y);ctx.rotate(angle);
  const fill=ctx.createLinearGradient(-size,0,size,0);
  fill.addColorStop(0,'#252c29');fill.addColorStop(.65,accent);fill.addColorStop(1,'#e5d9bc');
  ctx.fillStyle=fill;ctx.beginPath();ctx.moveTo(-size,0);
  ctx.quadraticCurveTo(-size*.15,-size*.42,size,0);
  ctx.quadraticCurveTo(-size*.25,size*.22,-size,0);ctx.fill();
  ctx.strokeStyle='#e1ccaf';ctx.lineWidth=.65;ctx.beginPath();ctx.moveTo(-size*.7,0);ctx.quadraticCurveTo(0,-size*.06,size*.85,0);ctx.stroke();
  ctx.restore();
}

function line(ctx:CanvasRenderingContext2D,points:Point[],width:number,color:string) {
...
export function drawNewMartialEffects(ctx:CanvasRenderingContext2D,feather:boolean,local:number,sample:(t:number)=>NewEffectFrame,dir:number,hitAge:number,hit:boolean,force:number,target:Point,definition?:EffectScore) {
  if(local<180 || local>1640) return;
  ...
  if(local>470 && local<980 && (signature || kind==='dive' || kind==='cascade')) {
    ...
    for(let i=0;i<4;i++) {
      ctx.globalAlpha=takeoff*(.18-i*.025);ctx.strokeStyle=i%2?accent:'#39433c';ctx.lineWidth=1.2+i*.25;
      ctx.beginPath();ctx.moveTo(ground[0]-dir*(30+i*14),327-i*3);
      ctx.bezierCurveTo(ground[0]+dir*50,285-i*6,f.root[0]-dir*60,f.root[1]-20-i*12,f.root[0]+dir*20,f.root[1]-70-i*8);ctx.stroke();
    }
  }
  for(const [start,end] of definition?.strokes || [[760,1180]]) if(local>start && local<end+60) {
    ...
    ctx.globalAlpha=opacity*.25*budget;ctx.fillStyle=accent;
    ctx.beginPath();ctx.moveTo(...frames[0].hand);
    for(const frame of frames) ctx.lineTo(...frame.tip);
    for(const frame of [...frames].reverse()) ctx.lineTo(...mix(frame.hand,frame.tip,.55));
    ...
    if(!signature) {
      ...
      if(kind==='point' || kind==='flurry') {
        const core=mix(f.hand,tip,.6),endPoint:Point=[tip[0]+Math.cos(angle)*22,tip[1]+Math.sin(angle)*22];
        line(ctx,[core,endPoint],3,accent);line(ctx,[core,endPoint],.8,'#fff5df');
        for(const sign of [-1,1]) {
          ctx.beginPath();ctx.moveTo(core[0]-Math.sin(angle)*sign*7,core[1]+Math.cos(angle)*sign*7);
          ctx.quadraticCurveTo(tip[0],tip[1],endPoint[0],endPoint[1]);ctx.strokeStyle=accent;ctx.lineWidth=.8;ctx.stroke();
        }
        ...
        for(let j=0;j<layers;j++) {
          ...
          ctx.beginPath();ctx.moveTo(origin[0]-dir*16,y+6);
          ctx.bezierCurveTo(origin[0]+dir*40,y-24-j*4,tip[0]-dir*22,y+12,tip[0]+dir*(24+phase*28),y-8);
          ctx.lineWidth=1.8-j*.4;ctx.strokeStyle=j%2?'#e5e9df':accent;ctx.stroke();
        }
      } else if(kind==='rise' || kind==='cascade') {
        const past=sample(start).tip;
        for(const sign of [-1,1]) {
          ctx.beginPath();ctx.moveTo(past[0]+dir*sign*9,past[1]);
          ctx.quadraticCurveTo(tip[0]-dir*(35+sign*8),past[1],tip[0]+dir*sign*12,tip[1]);
          ctx.strokeStyle=sign===1?accent:'#ebeddf';ctx.lineWidth=sign===1?2:1;ctx.stroke();
        }
      } else {
  ...
  if(hit && hitAge>=0 && hitAge<490) {
    ...
    for(let i=0;i<Math.round((feather?17:23)*budget);i++) {
      ...
      if(feather) featherParticle(ctx,x,y,angle+(dir<0?Math.PI:0)+age*.8,5+q*9,accent);
      else {
        line(ctx,[[x-dir*5*(1-age),y-3],[x+dir*(6+q*8)*(1-age),y+2]],1+q*2.8,i%3?accent:'#2b3936');
      }
    }
    if(!feather && (signature || kind==='surge')) for(const sign of [-1,1]) {
      ctx.globalAlpha=fade*.6;ctx.strokeStyle=accent;ctx.lineWidth=2.2*(1-age)+.3;
      ctx.beginPath();ctx.moveTo(target[0]+dir*spread*8,target[1]);
      ctx.quadraticCurveTo(target[0]+dir*spread*60,target[1]+sign*spread*65,target[0]+dir*spread*130,target[1]+sign*spread*48);ctx.stroke();
      ctx.globalAlpha=fade*.8;ctx.strokeStyle='#eeeede';ctx.lineWidth=.8;ctx.stroke();
    }
  }
  if(local>1110 && local<1530) {
    const age=unit((local-1110)/420),root=sample(1140).root;
    ctx.globalAlpha=(1-age)*.22*budget;ctx.strokeStyle=accent;
    for(let i=0;i<2;i++) {
      ctx.lineWidth=1.2-i*.3;ctx.beginPath();ctx.ellipse(root[0],329,18+age*(64+i*18),2+age*(6+i*2),0,0,Math.PI*2);ctx.stroke();
    }
  }
  ctx.restore();
}
